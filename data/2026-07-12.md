...
Why: 直接涉及CSI信道合成、波束赋形和物理层AI方法，但cGAN相比扩散模型/掩码自编码器等当前高优先级方法匹配度略低。

TL;DR: 提出了一种基于生成式AI增强的数字孪生框架，使用条件生成对抗网络（cGAN）进行主动式稀有事件信道合成，结合最坏情况迫零波束赋形实现分布鲁棒预编码，在室内超密集网络中实现了5-8 dB SINR增益和60-85%的完美CSI间隙闭合。


<details>
  <summary>Details</summary>
Matched topics: CSI acquisition, generative AI for wireless, channel synthesis, beamforming, digital twin

Motivation: 传统数字孪生无法预测移动性导致的遮挡和局部多用户热点带来的严重干扰，需要一种能够主动合成稀有事件信道的框架来提升网络鲁棒性。

Method: 1) 提出GenAI增强的DT框架，使用带时空生成器和PatchGAN判别器的cGAN进行主动式稀有事件信道合成；2) 采用基于蒙特卡洛合成轨迹的WC-ZF波束赋形器实现分布鲁棒预编码；3) 控制信道开销限制在每10ms时隙约2.1kB。

Result: Sionna仿真验证：中位SINR增益5-8 dB，丢包率降低60-70%，完美CSI oracle间隙闭合60-85%，推理开销2.8-4.1ms。

Conclusion: 生成式AI增强的数字孪生框架能有效处理超密集室内网络中移动性引起的干扰问题，通过稀有事件信道合成和分布鲁棒预编码显著提升性能。
...
Why: 论文关注RIS的空间部署优化（覆盖与吞吐量权衡），属于RIS系统级部署问题，但与CSI获取、信道估计、信道重构、CSI反馈压缩等核心研究方向无直接关联，AI方法仅体现为贝叶斯优化，并非深度学习/生成模型等现代AI方法在信道估计中的应用。

TL;DR: 本文研究RIS的物理空间部署策略，提出一个分层优化问题，在保证概率覆盖的前提下最大化期望和速率，并最小化部署RIS数量。方法包括：通过约束集覆盖问题在可见性图上获得最优部署区域，通过交集超图贪心划分确定最少RIS数量，最后用贝叶斯优化求解最终部署位置。


<details>
  <summary>Details</summary>
Matched topics: RIS部署, 贝叶斯优化

Motivation: 现有RIS部署研究主要关注最大化覆盖以绕过障碍物实现公平性，但未能利用用户密度的空间分布来提升吞吐量。本文希望在不牺牲吞吐量的前提下实现公平覆盖。

Method: 1) 构建分层非凸优化问题，最大化期望和速率同时保证概率覆盖；2) 在可见性图上求解约束集覆盖问题得到最优部署区域；3) 在交集超图上用贪心最小划分确定最少RIS数量；4) 使用贝叶斯优化计算最终RIS最优部署位置。

Result: 数值结果表明所提框架能够在没有不实际系统假设的情况下，一致地识别出同时实现良好覆盖和吞吐量的RIS部署位置。

Conclusion: 提出了一种兼顾覆盖和吞吐量的RIS空间部署框架，不依赖不实际的系统假设，能够有效确定最优RIS位置和数量。
...
Why: 虽然使用了掩码重建模型和频域损失等AI方法，但该论文聚焦于可穿戴PPG信号的SpO₂估计这一生物医学传感问题，与无线通信中的CSI估计/重建/压缩问题无直接关联。

TL;DR: 提出了一种SpO₂预测器引导的分阶段时频重建框架，用于低质量双波长PPG信号的重建，通过结合时域波形损失、频域STFT损失和预训练SpO₂预测器约束，在公开和私有数据集上取得了最低的SpO₂估计MAE。


<details>
  <summary>Details</summary>
Matched topics: 掩码重建模型, 频域损失, 传感器信号重建

Motivation: 现有PPG去噪重建方法主要优化波形保真度或心率特征，但仅靠时域波形损失无法充分保留频率结构和SpO₂相关信息，导致低质量PPG段上的SpO₂估计精度下降。

Method: 提出SpO₂预测器引导的分阶段时频重建框架：(1)选取高质量PPG段预训练SpO₂预测器；(2)训练掩码重建模型恢复随机掩码的PPG区域，联合优化时域波形损失和STFT频域损失；(3)引入预训练SpO₂预测器作为额外约束，使重建PPG保留SpO₂信息而非仅最小化波形误差；(4)四个训练阶段联合优化。

Result: 在公开OpenOximetry Repository数据集上达到2.882%的subject-level MAE，在私有可穿戴PPG数据集上达到2.359%的MAE，均为最低误差。

Conclusion: SpO₂预测器引导的时频重建框架能有效恢复低质量PPG信号的SpO₂相关信息，优于仅优化波形保真度的传统方法。
...
Why: 论文聚焦多频段ISAC的感知检测性能与资源优化，未涉及CSI获取、信道估计、CSI反馈/压缩、信道重构等核心方向，关联度较弱。

TL;DR: 提出多频段ISAC系统的分析性能表征与资源优化框架，推导多频段OFDM信号的检测和虚警概率闭式解，并通过ADMM算法联合优化功率和时频资源，在90%检测概率下相比单频段获得18dB增益。
...
Motivation: 多频段ISAC通过频率分集缓解频率选择性RCS导致的感知性能波动，但缺乏系统的理论分析和资源优化方法。

Method: 推导多频段OFDM检测与虚警概率闭式解；提出联合功率和时频资源分配方案，用ADMM算法求解。

Result: 闭式推导准确，多频段信号鲁棒性更优，优化方案在90%检测概率下比单频段基线提升18dB增益。

Conclusion: 多频段ISAC能显著提升检测性能，所提框架为多频段ISAC资源分配提供了理论依据。
...
Why: 论文聚焦于ISAC系统中的DoA估计问题，属于信号处理与阵列处理范畴，虽然涉及MIMO天线和子空间方法，但与CSI获取、信道估计、AI方法等核心研究方向无直接关联。

TL;DR: 提出了一种低复杂度的截断Hankel Newton-MUSIC框架，用于可重构天线阵列ISAC系统中的单快拍DoA估计，通过固定行维度的截断Hankel矩阵降低计算复杂度，结合牛顿更新消除网格量化误差，在大阵列场景下相比传统方法实现两个数量级以上的运行时间缩减。


<details>
  <summary>Details</summary>
Matched topics: MIMO, 阵列信号处理, ISAC

Motivation: 可重构天线阵列ISAC系统需要在短相干时间内进行高分辨率DoA估计，但单快拍场景下传统子空间方法面临秩亏问题，而基于Hankel的空间平滑方法在大阵列时计算成本过高，需要低复杂度的实时解决方案。

Method: 提出截断Hankel Newton-MUSIC方法：1) 构造固定行维度的截断Hankel矩阵恢复有效信号子空间，使复杂度与阵列规模呈线性关系；2) 粗网格估计后采用二阶牛顿更新在连续角度域精化估计，消除网格量化误差。

Result: 所提方法在大阵列场景下相比传统方形Hankel MUSIC实现两个数量级以上运行时间缩减，同时DoA估计精度接近全尺寸Hankel Newton-MUSIC方法。

Conclusion: 提出的低复杂度网格less截断Hankel Newton-MUSIC框架适合可重构天线ISAC系统的实时单快拍DoA估计，在保持精度的前提下大幅降低计算负担。
...
### [6] [Fundamental Sensing Limits of 6G Cooperative MIMO-ISAC Networks: Joint Position-Velocity CRLB and Decoupling Analysis](https://arxiv.org/abs/2607.08510)
...
Why: 本文聚焦于ISAC协作网络的CRLB性能分析，属于通信感知一体化理论，但未涉及AI方法、信道估计、CSI获取/反馈/压缩或信道知识建模等核心研究方向。

TL;DR: 本文对协作MIMO ISAC网络进行了基于CRLB的性能界分析，推导了位置和速度的完整FIM及精确CRLB，并提出了简化CRLB及其有效性判据。仿真验证了简化CRLB在一般情况下的适用性。


<details>
  <summary>Details</summary>
Matched topics: MIMO, ISAC

Motivation: 现有协作ISAC网络的CRLB分析大多忽略位置与速度之间的耦合，缺乏完整的FIM推导和简化判据，作者希望填补这一空白，提供更精确且易于优化的性能界。

Method: 推导协作MIMO ISAC中信号级参数到状态参数（位置、速度）的CRLB变换；构建完整FIM并考虑位置-速度耦合项；提出简化CRLB及有效性判据；涵盖多单站、多双站及混合感知模式。

Result: 仿真表明简化CRLB在一般情况下有效，并展示了资源和几何参数对位置/速度误差界的影响。

Conclusion: 简化CRLB具有低计算复杂度，可作为资源分配和波束成形等优化问题的可靠性能指标，适用于一般协作ISAC场景。
...
### [7] [RSMA-Assisted OFDM-OTFS Hybrid Framework for Mixed-Mobility Multiuser Systems](https://arxiv.org/abs/2607.08532)
...
Why: 论文涉及RSMA、OFDM/OTFS波形共存和信道估计误差分析，属于无线通信物理层问题，但与CSI获取、重建、压缩、反馈或AI辅助信道建模等核心方向关联较弱，AI方法非本文重点。

TL;DR: 本文提出一种RSMA辅助的6G车载网络系统，支持OFDM和OTFS用户共存，通过公共流帮助OFDM用户对抗高多普勒引起的ICI，并为OTFS用户分配专用频带。仿真分析了功率分配和信道估计误差对系统性能的影响。


<details>
  <summary>Details</summary>
Matched topics: 信道估计误差分析, MIMO/OFDM无线通信

Motivation: 6G车载网络中OFDM和OTFS用户可能共存于高/低移动性场景，OFDM面临ICI问题，OTFS占用更大频谱资源。需要一种灵活的多址接入框架来高效处理这些异构用户。

Method: 提出RSMA辅助的下行通信系统：公共流包含OFDM用户的公共消息并占用全带宽以辅助ICI管理；OTFS用户不参与公共流；OFDM私有流和OTFS流在不相交的频带上传输；所有接收端考虑信道估计误差进行SIC处理。

Result: 仿真结果展示了功率分配因子和信道估计误差对系统性能的影响，证明所提框架在中断概率和速率性能方面优于正交复用方案。

Conclusion: RSMA辅助框架能有效支持OFDM和OTFS异构用户的共存，通过公共流设计缓解OFDM的ICI问题，在不同移动性条件下优于传统正交复用方案。
...
Why: 论文聚焦寄生天线阵列的二进制控制与波束赋形码本设计，属于天线硬件与波束成形领域的创新，与CSI估计、反馈、压缩、信道重建或AI辅助物理层设计的核心研究主题关联较弱。

TL;DR: 提出了一种基于二进制控制的多有源多寄生天线（MAMP）架构，每个寄生元件在两种离散电抗状态间切换。通过HFSS仿真验证阵列几何结构，将感应电流向量建模为二进制状态向量的二次函数，并设计了两类基于广义Lloyd算法的发射波束赋形码本：穷举搜索（理论上限）和基于特征值扰动（低复杂度在线适配）。仿真表明，少量有源天线的MAMP可实现接近全有源天线阵列的波束赋形性能。
...
Motivation: 6G MIMO天线规模持续增长，但射频链路的成本、功耗和硬件复杂度限制了实际部署。寄生天线可增加空间自由度而不成比例增加RF链路，但现有基于变容管的连续电抗调谐方式在模拟控制和RF电路设计上仍存在高成本和高复杂度问题，因此亟需更实用的二进制控制方案。

Method: 提出MAMP架构，每个寄生元件仅使用两种离散电抗状态（二进制控制）；通过HFSS仿真识别与实际辐射方向图匹配的阵列几何；将感应电流向量建模为二进制状态向量的二次函数，选取最小化相对误差的离散电抗值；基于广义Lloyd算法设计两种波束赋形码本：一种穷举所有二进制组合（理论上限），另一种利用特征值扰动降低计算复杂度以支持在线适配。

Result: 仿真结果表明，所提出的码本设计使仅含少量有源天线的MAMP架构能够实现与拥有更多有源天线的全有源天线阵列相当的波束赋形性能。

Conclusion: 二进制控制的MAMP架构是一种在性能和复杂度之间取得良好折衷的实用天线设计方案，其特征值扰动码本可在保持竞争力的同时大幅降低计算开销，适合在线部署。
...
Why: 论文聚焦于物理层安全中的混沌调制和熵调控，未涉及CSI获取、信道估计、反馈压缩或AI辅助物理层设计等核心研究主题。

TL;DR: 提出了一种多地图动态熵入侵感知混沌调制框架（MU-DE-IAEACM-MM），通过熵驱动的自适应混沌参数更新和异构混沌映射分配，实现多用户无线系统中的自适应物理层安全，并在多种干扰场景下验证了保密容量增益。
...
Motivation: 传统基于静态参数保密的混沌方案安全性不足，需要一种能够动态调节熵值以抵御窃听者攻击的自适应物理层安全机制。

Method: 将熵作为受控安全变量，采用熵驱动自适应律动态更新混沌控制参数；为用户分配Logistic、Tent、Chebyshev、Sine等异构混沌图以扩大熵维度；引入基于相关性的入侵检测指标来触发熵值升级。

Result: 在AWGN、衰落、脉冲、有色和窄带干扰等多种信道下，合法接收者与不匹配接收者之间存在持续的BER差距，相比固定参数混沌调制方案有可量化的保密容量提升，在64用户和30个被动窃听者场景下仍能保持正保密速率。

Conclusion: 熵调控的多图混沌调制为下一代无线和IoT网络提供了一种可扩展且同步稳定的自适应物理层安全方案。
...
Why: 本文聚焦于自动调制分类（AMC）的跨域泛化，虽涉及信号表示和对抗域适应，但与CSI获取、信道估计、CSI反馈/压缩、RIS/mmWave信道等核心研究方向无直接关联。

TL;DR: 本文提出一种融合信号先验知识与深度学习数据的双驱动网络（DKDNet），用于跨域自动调制分类（AMC）。通过分析五种信号表示并选出IQ、AP、ACF作为先验输入，设计多表示特征编码器（MRFE）和动态轻量融合单元（DLFU）实现统一表示学习和自适应特征融合，结合分类与域对齐目标进行优化。
...
Motivation: 通信环境的动态性导致域分布偏移，使得深度学习AMC模型泛化困难。现有无监督域适应（UDA）方法没有充分利用在不同域条件下仍保持信息量的调制特定结构，因此需要引入信号先验知识来增强跨域表示学习。

Method: 1) 分析五种信号表示（IQ、AP、ACF等）的调制判别性、域稳定性和互补性；2) 选取IQ、AP、ACF作为先验引导输入；3) 设计多表示特征编码器（MRFE）进行统一表示学习；4) 设计动态轻量融合单元（DLFU）实现自适应特征融合；5) 结合调制分类损失和对抗域对齐目标联合优化。

Result: 在仿真和公开数据集上的实验验证了先验选择的合理性，并证明了所提方法在跨域AMC任务上的优越性能。

Conclusion: 信号先验知识可以有效增强跨域调制分类的表示学习，提出的DKDNet通过多表示融合和动态融合机制实现了更好的域泛化性能。
...
Why: 论文涉及无线通信物理层的新型接收器模型，但聚焦于量子干扰路径的等效信道建模，完全不涉及AI方法、CSI估计/反馈/压缩、MIMO/OFDM/mmWave信道估计或任何机器学习技术，与研究核心方向无交集。

TL;DR: 本文为原子接收器建立了统一的分析模型，提出了量子相干转移系数（QCTC）来表征三种典型量子干涉路径（V型、Λ型、Ξ型）的等效信道响应，并推导了等效信道增益的闭式表达式，为原子无线电通信的容量分析和波形优化提供了理论基础。
...
Motivation: 现有原子接收器的信息论表征局限于Ξ型EIT路径且依赖弱探针近似，无法预测高信噪比下的行为，缺乏统一的解析模型来描述不同量子干涉路径的等效信道响应。

Method: 基于三能级EIT稳态微扰框架，提出量子相干转移系数（QCTC）来建模原子接收器的等效信道响应，推导了V型、Λ型、Ξ型三种路径的等效信道增益闭式表达式。

Result: 建立了原子接收器的统一分析模型，得到了三种量子干涉路径的等效信道增益闭式解，为后续容量分析和波形优化提供了解析基础。

Conclusion: 本文为原子无线电通信提供了理论基础，但主要关注原子接收器的物理层建模，未涉及AI方法或传统CSI问题。
...
Why: 本文聚焦于里德伯原子接收机的硬件架构和频率跟踪算法，不涉及MIMO/OFDM/massive MIMO等无线通信系统的信道估计、CSI获取/反馈/压缩、或任何AI方法，与研究画像的核心方向无交集。

TL;DR: 本文提出一种自适应本振跟踪里德伯原子接收机架构，利用交叉积自动频率控制算法动态估计多普勒频偏并通过反馈环路调整本振频率，使中频信号始终锁定在原子响应带宽中心，显著缓解高动态环境下的性能恶化。
...
Motivation: 里德伯原子接收机具有超高灵敏度，但瞬时带宽窄；在高动态场景（如卫星通信）中，严重多普勒效应导致载波频偏超出瞬时带宽，造成信号严重失真，现有架构无法解决这一问题。

Method: 提出自适应本振跟踪接收机架构，采用交叉积自动频率控制（CPAFC）算法实时估计瞬时频偏，生成误差控制信号，通过反馈环路动态调整本振频率，使中频信号始终锁定在原子响应带宽中心附近。

Result: 仿真结果表明，所提架构显著优于现有里德伯原子接收机，有效缓解了高动态环境下的性能退化。

Conclusion: 自适应本振跟踪架构能够解决里德伯原子接收机在高动态场景下的频偏锁定问题，提升其在卫星通信等场景的实用性。
...
Why: 论文主要关注小波调制和自适应多用户资源分配，涉及信道均衡但未涉及CSI的获取、重建、反馈、压缩或AI方法，与研究重点不匹配。

TL;DR: 提出了一种自适应小波分复用方案，利用离散小波变换的多分辨率特性，为不同移动速度的用户分配不同的分解层级，在频率选择性瑞利衰落信道上实现性能均衡，相比OFDM和OTFS有更低的PAPR。
...
Motivation: 解决异构移动性用户在同一无线系统中共存时，传统OFDM和OTFS调制方案难以同时适应不同多普勒条件的用户，导致性能不均衡的问题。

Method: 基于离散小波变换(DWT)的多分辨率结构，根据用户的信道动态和多普勒条件，将用户自适应地分配到不同的分解层级。在频域使用单抽头MMSE均衡器进行均衡。

Result: 实现了所有移动性类别用户间均衡的误码率(BER)，并相比传统OFDM和OTFS获得了显著的峰均比(PAPR)降低。在四用户异构移动性场景中验证了可扩展性。

Conclusion: 自适应小波分复用方案能有效处理异构多用户移动性场景，在BER均衡和PAPR降低方面优于OFDM和OTFS，具有良好的可扩展性。
...
### [14] [Low-Rate Wrist SpO2 Estimation under Micro-Perturbations Using Motion-Aware Beat Selection and Perfusion-Guided Calibration](https://arxiv.org/abs/2607.08001)
...
Why: 论文聚焦于可穿戴健康传感中的SPO2估计，与无线通信中的CSI获取、信道估计、MIMO/OFDM/RIS/mmWave信道或AI物理层设计无任何关联。

TL;DR: 提出了一种轻量级低采样率腕部血氧饱和度(SPO2)估计框架，结合运动感知心跳选择和灌注引导校准，在25 Hz采样率下实现了与100 Hz相当的估计性能，同时降低了功耗。
...
Motivation: 腕部PPG信号的SPO2估计受微扰动和个体灌注差异影响，传统固定R-SPO2映射可靠性不足，且高采样率导致功耗较高，亟需一种低功耗、鲁棒的估计方法。

Method: 从双波长PPG信号提取心跳级AC/DC分量，计算心跳级R值；利用加速度计导出的运动分数对滑动窗口内的心跳进行加权；使用个体特异性灌注参考进行跨灌注条件的校准。

Result: 在私有可穿戴数据集上，25 Hz采样率下MAE为2.305±1.113%，RMSE为3.117±1.743%，性能与100 Hz采样率相当，且显著降低了PPG传感器功耗。

Conclusion: 该框架有效实现了低采样率下的鲁棒腕部SPO2估计，适用于能量受限的可穿戴设备。
...
Why: 该论文研究可穿戴设备上的GSR信号处理与压力检测，属于生理信号处理和健康监测领域，与无线通信、CSI获取/重构/压缩/反馈、MIMO/mmWave/RIS信道估计等核心研究方向完全无关。

TL;DR: 提出了一种与单位无关的低速率腕部GSR处理流水线，用于提取每分钟皮肤电反应次数(nSCR/min)作为压力相关特征。在31名受试者中收集配对腕部和手掌GSR数据，通过随机森林分类器在TSST与坐姿/站姿基线之间的二分类任务中分别达到0.823和0.871的平衡准确率。
...
Motivation: 腕部GSR的绝对幅度与实验室级别的掌部测量存在显著差异，需要一种与单位无关、低采样率的处理方法来提取可靠的压力相关特征，以实现可穿戴设备的压力检测。

Method: 提出低速率腕部GSR处理流水线：原始GSR信号清洗→分解为紧张性SCL和相位性SCR→鲁棒z-score归一化→检测相位性SCR峰值并计算nSCR/min。使用随机森林分类器进行二分类任务。

Result: 在TSST与坐姿基线、站姿基线的二分类中，平衡准确率分别为0.823和0.871。25Hz We-Be GSR特征与原始100Hz特征性能相当。
...
Why: 直接命中多个高优先级主题：扩散模型/流匹配用于无线信道预测、几何/环境感知的无线电地图构建、6G/MIMO波束选择、以及零样本泛化能力，且方法实用性强（单模型多任务、贝叶斯最优决策），与研究者对AI+无线物理层的核心兴趣高度吻合。

TL;DR: 本文提出RadioDiff-v2，一种双分支一维扩散Transformer模型，用于从几何信息预测角功率谱（APS）以实现6G网络中的波束选择和接收机定位。该方法将任务建模为感知-失真问题，通过流匹配训练，结合周期角编码、自适应层归一化、傅里叶角混合器以及联合速度与干净信号头。理论证明了集中条件分布可产生直的概率流轨迹，单步积分即可精确求解。在99个环境的零样本测试中，在所有指标上超越所有基线。


<details>
  <summary>Details</summary>
Matched topics: 扩散模型 / 流匹配, 无线电地图 / 角功率谱, 波束选择, 接收机定位, 6G无线网络, 环境泛化 / 零样本, 生成式信道建模, 感知-失真权衡

Motivation: 从几何信息预测角功率谱（APS）是一个病态问题，特别是在非视距（NLOS）条件下，且需泛化到未见环境。传统最小失真回归器返回条件均值，会过度平滑频谱并抹除多径结构，损害下游任务性能。

Method: 提出RadioDiff-v2，一种双分支一维扩散Transformer，使用流匹配（flow matching）训练。包含周期角编码、自适应层归一化条件化、傅里叶角混合器、以及联合速度头和干净信号头。利用每指标估计器组合，从单一模型同时提供分布采样、回归级点估计、贝叶斯最优波束选择和条件似然定位。理论上证明集中条件分布产生直概率流轨迹，单步积分精确可解。

Result: 在99个环境和100万链路的零样本测试中，RadioDiff-v2在所有指标上领先所有基线：Wasserstein-1距离0.39 dB，每bin误差低于回归基线，8波束NLOS扫描损失2.43 dB，4基站定位误差20.6像素。

Conclusion: RadioDiff-v2通过将APS预测重新定义为感知-失真问题并采用扩散Transformer和流匹配，在零样本泛化中显著超越现有方法，为6G网络中的波束选择和定位提供了实用高效的解决方案。
...
Why: 论文关注OTFS系统中的信道估计（IOR估计），属于无线通信物理层问题，但未涉及AI方法，且系统为Zak-OTFS而非CSI密集型MIMO/大规模MIMO系统，与研究者核心方向（AI驱动的CSI获取、压缩、推理）关联较弱。

TL;DR: 该论文研究多用户Zak-OTFS系统的上行链路，提出基于TF移位的多址接入方案，推导了有效DD域信道闭式表达式，并采用叠加扩散导频框架进行输入输出关系估计，通过DD字典迭代估计与数据检测来抑制导频-数据干扰。
...
Motivation: 解决多用户Zak-OTFS系统中用户具有异质延迟-多普勒周期/帧尺寸时的信道估计与多址接入问题。

Method: 采用TF移位实现多址接入，推导sinc和高斯脉冲整形下DD域信道闭式表达式；使用基于ZC序列的扩散导频进行IOR估计，并通过DD字典迭代IOR估计与数据检测来抑制导频-数据干扰。

Result: 仿真表明所提方案NMSE和BER性能接近单用户系统；sinc脉冲下叠加扩散导频帧频谱效率更高，高斯脉冲下嵌入导频帧更优；方案对信道功率延迟分布和多普勒频移变化具有鲁棒性。

Conclusion: TF移位多址能有效解耦多用户IOR估计问题，叠加扩散导频方案在sinc脉冲下具有优势，但高斯脉冲下受残余IUI和导频-数据干扰影响。
...
### [18] [Optimization and Deep Learning based Resource Allocation for UAV-Aided Wireless Communication with Rotatable Antenna Array](https://arxiv.org/abs/2607.08420)
...
Why: 论文涉及多天线无人机通信中的波束赋形和天线朝向优化，虽使用GNN加速求解，但核心问题不是CSI估计、重构、反馈或压缩，与给定的研究画像中CSI核心方向关联较弱。

TL;DR: 本文提出了一种配备可旋转天线阵列（RAA）的无人机通信系统，通过联合优化RAA朝向和波束赋形来提升多用户和速率。作者提出了基于罚对偶分解（PDD）的优化框架和图神经网络（GNN）的深度学习框架，仿真表明RAA优于固定天线阵列，且GNN方法在保持相近和速率的同时显著降低计算时间。


<details>
  <summary>Details</summary>
Matched topics: graph neural networks, MIMO, beamforming

Motivation: 传统固定天线阵列（FAA）在无人机通信中无法灵活调整天线朝向以对准用户，导致空间自由度利用不充分、用户间空间相关性高。本文旨在通过可旋转天线阵列（RAA）引入额外的空间自由度，提升多用户信号传输和干扰管理能力。

Method: 1) 建立RAA朝向与波束赋形的联合优化问题，以最大化多用户和速率为目标，满足每用户QoS约束；2) 提出基于罚对偶分解（PDD）的迭代优化框架求解非凸问题；3) 提出基于图神经网络（GNN）的深度学习框架加速求解，实现实时部署。

Result: 仿真表明RAA显著优于FAA；PDD优化框架能高可靠地满足QoS要求；GNN深度学习框架在保持相近和速率性能的同时大幅降低计算时间，并对用户位置误差具有鲁棒性。

Conclusion: RAA为无人机通信提供了额外空间自由度，联合优化RAA朝向与波束赋形可有效提升系统性能。GNN加速方案在实时部署和鲁棒性方面具有实际优势。
...
Why: 本文聚焦穿墙雷达人体活动识别（TWR HAR）的泛化分析，属于雷达感知/HAR领域，未涉及MIMO/massive MIMO/OFDM/mmWave/THz等无线通信信道估计、CSI获取、反馈压缩或AI辅助物理层设计，与研究档案核心方向无直接关联。

TL;DR: 本文针对穿墙雷达人体活动识别（TWR HAR）中因人员、视角和墙体变化导致的结构化分布偏移问题，提出了一套泛化分析框架，统一建模了人体运动学、雷达回波、图像形成、特征表示和有界权重神经网络，推导了目标域泛化界，并分析了物理低维表示、多源训练和参数空间覆盖对收紧泛化界的作用。
...
Motivation: 现有TWR HAR方法在跨人员、跨视角、跨墙体等结构化分布偏移下泛化能力严重下降，且缺乏对目标域误差来源的严格理论解释。

Method: 建立从源域到目标域的统一学习框架，建模人体运动学、雷达回波生成、雷达图像形成、特征表示和有界权重神经网络；定义源风险、目标风险、经验风险和可容许物理域描述子，推导目标域泛化界；将结构化偏移分解为跨人、跨视角、跨墙体三部分，分析物理低维表示、多源训练和参数空间覆盖对收紧泛化界的作用。

Result: 通过仿真与实测实验验证了理论分析的有效性和应用价值。

Conclusion: 该泛化分析框架为TWR HAR在结构化分布偏移下的泛化提供了理论解释和指导。
...
Why: 该论文聚焦于机器人控制中的传感-通信-计算-控制闭环带宽分配和DRL优化，属于控制与通信资源分配领域，未涉及CSI获取、信道估计、CSI反馈/压缩、MIMO/mmWave/RIS信道建模或AI辅助物理层设计等核心研究方向。

TL;DR: 本文针对远程/危险区域的机器人控制任务，提出了一种基于深度强化学习（PPO）的传感器到边缘信息中心（EIH）带宽分配方案，以最小化线性二次型调节器（LQR）控制代价。通过互信息理论建模有限通信数据率导致的失真噪声，并将控制过程建模为部分可观测马尔可夫决策过程（POMDP）进行优化。
...
Motivation: 机器人部署在远程或危险区域时，个体能力有限，需要依赖其他现场传感器获取目标状态信息，并通过边缘信息中心（EIH）实现信息交换、感知数据分析和控制命令生成。如何优化传感-通信-计算-控制（SC3）闭环中的带宽分配以提升整体控制性能是核心问题。

Method: 基于互信息理论建模传感-通信链路中有限数据率引起的失真噪声；采用卡尔曼滤波和LQR控制器构建控制策略；将问题建模为POMDP；利用PPO算法训练DRL智能体进行传感器到EIH的带宽分配优化。

Result: 仿真结果表明，所提出的基于DRL的带宽分配方案相比基准方案具有优越的控制性能。

Conclusion: 所提DRL方案能有效优化传感-通信-计算-控制闭环中的带宽资源配置，降低LQR控制代价，适用于远程/危险区域的机器人控制场景。
...
### [21] [Why Constants Matter in Distribution Testing: From Uniformity to Calibration](https://arxiv.org/abs/2607.08378)
*Alon Kipnis*
...
Why: 该论文纯属统计学中的分布拟合优度检验理论，不涉及无线通信、CSI、信道估计、AI方法或任何物理层问题。

TL;DR: 该论文讨论了分布拟合优度检验中的常数级（constant-level）理论，主张在率级（rate-level）最优检验中进一步区分不同检验的功效，并以大规模字母表均匀性检验和校准检验中分箱数选择为例阐明这一观点。
...
Motivation: 率级理论已能回答分布检验所需样本量随字母表大小、与零假设的分离度等如何缩放，但无法在多个率级最优的检验中区分谁的实际风险或检验功效更好。现代应用中分布检验作为实用设计工具，需要回答这一常数级问题。

Method: 论文通过理论分析，将分布检验中的sharp常数类比于参数估计中的Fisher信息和非参数估计中的Pinsker常数，说明它们能揭示有效信噪比、指导下游调参，并以大规模字母表均匀性检验和校准检验的分箱数选择为例说明。

Result: 指出sharp常数可以：(1) 区分率级相同但功效不同的检验；(2) 揭示控制检验问题的有效信噪比；(3) 指导下游应用中的调参选择。

Conclusion: 分布检验的常数级分析在理论和实践中具有重要意义，能提供率级理论无法提供的精细化指导。
...
### [22] [On the Convergence of Belief Propagation for Multipath Data Association in Target Tracking](https://arxiv.org/abs/2607.08521)
...
Why: 本文聚焦于目标跟踪中的数据关联问题，属于雷达/多目标跟踪领域，未涉及无线通信中的信道估计、CSI获取、MIMO/OFDM信道建模或AI辅助物理层设计等核心研究方向。

TL;DR: 本文证明了多路径数据关联（MPDA）中置信传播（BP）更新的收敛性，建立了唯一不动点的收敛保证，并通过仿真验证了BP在MPDA中的收敛行为及其相对于多检测多假设跟踪器的精度-效率权衡优势。
...
Motivation: 多路径数据关联（MPDA）中目标、路径和测量之间存在三维对应关系，现有的BP收敛性分析仅适用于二维对应（目标和测量），尚未提供MPDA场景下的完整收敛证明。

Method: 对多路径数据关联（MPDA）中的置信传播更新提出了完整的收敛性证明，建立了唯一不动点的收敛保证，并通过仿真验证收敛行为。

Result: 证明了MPDA中BP更新收敛到唯一不动点，仿真显示BP在MPDA中具有良好的收敛行为，并在精度-效率权衡上优于多检测多假设跟踪器变体。

Conclusion: 为MPDA中的BP提供了收敛性理论保证，验证了其在实际应用中的有效性。
...
### [23] [New sharp inequalities involving non-relative, relative and cross informational functionals with some remarkable minimizers of generalized Gaussian and Beta types](https://arxiv.org/abs/2607.08599)
...
Why: 该论文纯粹是信息论/数学方向的抽象不等式推导，没有涉及无线通信、CSI获取、信道估计、AI方法或任何物理层无线问题，与所描述的研究方向无直接关联。

TL;DR: 该论文推导了若干新的信息不等式，包括类Stam不等式、矩熵类不等式，以及涉及Rényi熵、Rényi散度和Rényi交叉熵的不等式，所有不等式均为紧致的，其最小化器为高斯、拉伸高斯或广义Beta分布的概率密度。
...
Motivation: 在相对框架下推导新的信息不等式，建立Rényi熵功率、缩放不变的相对Fisher信息与Rényi交叉熵之间的理论联系。

Method: 利用类Stam不等式和矩熵类不等式在相对框架下的推广，以及最近建立的混合Rényi熵、Rényi散度和Rényi交叉熵的不等式进行分析推导。

Result: 得到了一个连接Rényi熵功率、相对Fisher信息和Rényi交叉熵的类Stam不等式；得到了仅涉及Fisher类信息测度的不等式；得到了仅涉及矩类泛函（包括非相对、相对和交叉类型）的不等式。

Conclusion: 所有不等式均为紧致的；类Stam不等式的最小化器在特定情况下为高斯或拉伸高斯概率密度；矩类不等式的最小化器为广义Beta分布的概率密度。
...
### [24] [Smoothing Exponents and Decoupling in Semifinite von Neumann Algebras](https://arxiv.org/abs/2607.07997)
...
Why: 论文涉及量子信息论与算子代数，与无线通信、CSI获取/估计/压缩/反馈、AI辅助物理层设计等研究主题无任何关联。

TL;DR: 本文研究半有限冯·诺依曼代数中最大相对熵的光滑指数，给出了精确指数公式，并证明了该指数由冯·诺依曼代数结构而非矩阵维度决定。作为应用，推导了具有半有限冯·诺依曼代数参考系统的催化量子信息解耦的可靠性指数公式。
...
Motivation: 将有限维量子信息论中的光滑熵指数公式推广到半有限冯·诺依曼代数框架下，克服有限维证明中依赖维度估计的工具限制。

Method: 发展算子代数工具替代有限维论证中的维度依赖方法；证明适用于冯·诺依曼代数的内层层（layer-cake）引理，去掉有限维证明中的可数谱假设。

Result: 得到了半有限冯·诺依曼代数中最大相对熵光滑指数的精确公式；催化量子信息解耦的可靠性指数由夹层Rényi互信息公式描述。

Conclusion: 光滑指数由冯·诺依曼代数结构而非矩阵维度决定，推广了有限维量子信息理论的核心结果到更一般的算子代数框架。
...
Why: 这是一篇纯粹的代数编码理论论文，研究有限非链环上的斜多循环码和常循环码，不涉及无线通信、信道估计、CSI或任何AI方法。

TL;DR: 这篇论文研究了有限非链环 R_{u^2,v^2,p^m} 上的斜多循环码和斜 (λ,Θ)-常循环码，描述了这些码的结构、表征了自由斜多循环码、确定了它们的秩，并给出了最优码的实例。
...
Motivation: 将有限域上的常循环码理论推广到更一般的有限非链环上，扩展已有工作在特定环上的结果。

Method: 通过中心多项式分解商环，利用自同构 Θ 构造斜多项式环，分析左理想结构来研究斜 (λ,Θ)-常循环码。

Result: 给出了自由斜多循环码的表征和秩的确定方法，将常循环码研究从特定环推广到更一般的非链环。

Conclusion: 本文建立了非链环上斜循环码的理论框架，扩展了前人在更简单环上的工作。
...
### [26] [Learning LDPC codes with quantized density evolution over relaxed protographs](https://arxiv.org/abs/2607.08484)
...
Why: 本文研究LDPC码的原模图设计，属于信道编码领域，不涉及CSI获取、信道估计、CSI反馈/压缩、RIS/mmWave信道或AI辅助无线物理层等核心研究方向。

TL;DR: 本文提出一种基于梯度下降的确定性框架，直接在松弛的原模图表示上优化LDPC码，利用密度进化（DE）的误码率作为损失函数，无需蒙特卡洛估计。实验表明优化后的原模图在相同维度下优于5G LDPC码。
...
Motivation: 现有的LDPC码设计方法（如遗传算法、基于梯度的解码器回路优化）存在参数调优困难、计算成本高、依赖噪声蒙特卡洛估计等问题，需要更高效、确定性的优化方法。

Method: 提出一种确定性梯度下降框架，将原模图条目松弛为概率值，基于密度进化（DE）的误码率（BER）作为可微损失函数，直接对松弛原模图进行梯度优化，无需解码器仿真或蒙特卡洛采样。

Result: 在min-sum解码器上的数值实验表明，优化后的原模图LDPC码在相同维度下优于5G LDPC码，且优化过程收敛快速稳定。

Conclusion: 所提确定性GD框架为长原模图LDPC码设计提供了一种高效、自主且可靠的优化途径，优于现有5G标准方案。
...
### [27] [Locality of Curve-Decoding and Improved Proximity Gaps](https://arxiv.org/abs/2607.08516)
...
Why: 本文完全属于编码理论、交互式证明和零知识论证的理论计算机科学范畴，与无线通信、信道估计、CSI获取或任何AI方法无关。

TL;DR: 本文针对随机编码族（随机线性码、随机Reed-Solomon码、Gallager LDPC码）获得了改进的邻近间隙（proximity gaps），将其提升至与子空间设计码相当的水平。作者通过扩展行跨度约束下的局部坐标线性（LCL）属性框架，将曲线可解码性（curve-decodability）直接建模为LCL属性，实现了从子空间设计码到随机码族的黑盒迁移。
...
Motivation: Goyal和Guruswami近期在交互式预言证明（IOP）和简洁非交互零知识论证（SNARK）的背景下，为多种编码族建立了近最优的邻近间隙，但随机编码族的参数劣于子空间设计码且随维度增加而退化。本文旨在消除这一差距。

Method: 扩展了Levi、Mosheiff和Shagrithaya以及Brakensiek等人提出的局部坐标线性（LCL）属性框架，引入"行跨度约束"版本，将曲线可解码性直接表述为行跨度约束LCL属性，而非使用代理属性。

Result: 对随机线性码、随机Reed-Solomon码和Gallager LDPC码获得了与子空间设计码相同的邻近间隙参数，且建立了从子空间设计码到随机码族的黑盒迁移机制。

Conclusion: 子空间设计码上的任何进展将自动转化为随机码族的对应进展，为IOP和SNARK中的邻近间隙研究提供了统一的框架。
